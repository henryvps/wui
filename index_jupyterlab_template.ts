...
class OpenTemplateWidget extends Widget {
  constructor() {
    ...
    label.textContent = 'Deploy scenario:';
...
function activate(app: JupyterLab,
  ...
  console.log('PALETTE:');
  console.log(palette);
  //let pallete = new CommandPalette({ commands });
  ...
  const command: string = 'faas:new';
  app.commands.addCommand(command, {
    label: 'Some FAAAAAAAS',
    execute: () => {
  ...
  palette.addItem({command, category: 'FAAS'});
